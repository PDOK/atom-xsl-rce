...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:georss="http://www.georss.org/georss">
						...
						<tr>
							...
							<td>
								<xsl:variable name="uuid">
									<xsl:apply-templates select="atom:feed/atom:link[@rel='describedby']"
										disable-output-escaping="yes" />
								</xsl:variable>
								<a href="{atom:feed/atom:link[@rel='describedby']/@href}">XML</a> / 
								<a
									href="https://nationaalgeoregister.nl/geonetwork/srv/dut/catalog.search#/metadata/{$uuid}">HTML</a>
							</td>
						</tr>
						</xsl:if>
						<tr>
	...
	<xsl:template match="atom:feed/atom:link[@rel='describedby']">
		<xsl:variable name="url" select="current()/@href" />
		<xsl:choose>
			<xsl:when test="contains($url, 'uuid=')">
				<xsl:value-of select="substring-after($url,'uuid=')" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="result" select="substring-after($url, 'id=')" />
				<xsl:choose>
